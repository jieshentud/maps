...
void GetNumElements(int N, double fN, int* iNumElementsOCL, int* iOffsetOCL, int iLwSizeOCL, int* iNumElementsSMP, int* iOffsetSMP, int iLwSizeSMP)
{
	...
	*iOffsetSMP = *iNumElementsOCL;
	...
	//this is not necessary, let the CPU processing the remaining part
	//if(*iNumElementsSMP % iLwSizeSMP != 0){
	//	int temp = RoundUp(iLwSizeSMP, *iNumElementsSMP);
	//	*iNumElementsSMP = temp;
	//}
	//printf("after rounding, *iNumElementsSMP = %d\n", *iNumElementsSMP);
